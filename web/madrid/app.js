async function loadFlights() {
  ...
  // ✅ 1) Preferimos cargar JSON relativo a la carpeta actual (/madrid/, /barcelona/, ...)
  // Esto evita hardcodear el market y evita dependencias de S3 si ya publicas los JSON en la web.
  const localFlightsUrl = "./flights_of_the_day.json";

  // ✅ 2) Fallback a S3 por si el JSON local no está (opcional pero útil)
  ...
  try {
    ...
    const recent = flights.slice(1, 5);
...
function getDiscountPct(flight) {
  ...
  if (flight.route_typical_price != null && flight.price_eur != null) {
    ...
    if (habitual > 0 && price > 0) return Math.max(0, (1 - price / habitual) * 100);
  }
  return null;
}
